// SPDX-License-Identifier: GPL-2.0
/*
 * Copyright (C) 2020 Marek Vasut <marex@denx.de>
 *
 * Based on rpi_touchscreen.c by Eric Anholt <eric@anholt.net>
 * Adapted for out-of-tree build: manual brightness control via sysfs
 */

#include <linux/err.h>
#include <linux/gpio.h>
#include <linux/gpio/driver.h>
#include <linux/i2c.h>
#include <linux/init.h>
#include <linux/interrupt.h>
#include <linux/module.h>
#include <linux/regmap.h>
#include <linux/regulator/driver.h>
#include <linux/regulator/machine.h>
#include <linux/regulator/of_regulator.h>
#include <linux/slab.h>

#include <linux/device.h>
#include <linux/sysfs.h>

#define REG_PWM      0x01
#define REG_IODIR    0x02
#define REG_OUTPUT   0x0A

#define PIN_LCD_BL_EN    BIT(0)
#define PIN_LCD_PWM      BIT(1)
#define PIN_LCD_RST      BIT(2)
#define PIN_TP_RST       BIT(3)
#define PIN_LCD_VDD_EN   BIT(4)

enum gpio_signals {
    LCD_BL_EN_N,
    LCD_BL_PWM_N,
    LCD_RST_N,
    TP_RST_N,
    LCD_VDD_EN_N,
    NUM_GPIO
};

struct gpio_signal_mappings {
    unsigned int reg;
    unsigned int mask;
};

static const struct gpio_signal_mappings mappings[NUM_GPIO] = {
    [LCD_BL_EN_N]  = { REG_OUTPUT,     PIN_LCD_BL_EN   },
    [LCD_BL_PWM_N] = { REG_OUTPUT,     PIN_LCD_PWM     },
    [LCD_RST_N]    = { REG_OUTPUT,     PIN_LCD_RST     },
    [TP_RST_N]     = { REG_OUTPUT,     PIN_TP_RST      },
    [LCD_VDD_EN_N] = { REG_OUTPUT,     PIN_LCD_VDD_EN  },
};

struct attiny_lcd {
    struct mutex      lock;
    struct regmap    *regmap;
    bool              gpio_states[NUM_GPIO];
    u8                port_states;
    struct gpio_chip  gc;
};

static const struct regmap_config attiny_regmap_config = {
    .reg_bits       = 8,
    .val_bits       = 8,
    .disable_locking = 1,
    .max_register   = REG_OUTPUT,
    .cache_type     = REGCACHE_RBTREE,
};

static int attiny_set_port_state(struct attiny_lcd *state, int reg, u8 val)
{
    state->port_states = val;
    return regmap_write(state->regmap, reg, val);
}

static u8 attiny_get_port_state(struct attiny_lcd *state, int reg)
{
    return state->port_states;
}

static int attiny_set_bit(struct attiny_lcd *state,
                          unsigned int reg,
                          unsigned int pin,
                          bool enabled)
{
    unsigned int mask = BIT(pin);
    unsigned int val  = enabled ? 0xffff : 0x0000;
    return regmap_update_bits(state->regmap, reg, mask, val);
}

/*
 * Simple sysfs interface to control backlight brightness (PWM register)
 */
static ssize_t brightness_show(struct device *dev,
                               struct device_attribute *attr,
                               char *buf)
{
    struct attiny_lcd *state = i2c_get_clientdata(to_i2c_client(dev));
    unsigned int val;
    int ret;

    ret = regmap_read(state->regmap, REG_PWM, &val);
    if (ret)
        return ret;
    return sprintf(buf, "%u\n", val);
}

static ssize_t brightness_store(struct device *dev,
                                struct device_attribute *attr,
                                const char *buf,
                                size_t count)
{
    struct attiny_lcd *state = i2c_get_clientdata(to_i2c_client(dev));
    unsigned int val;
    int ret;

    ret = kstrtouint(buf, 0, &val);
    if (ret)
        return ret;

    ret = regmap_write(state->regmap, REG_PWM, val);
    if (ret)
        return ret;

    return count;
}
static DEVICE_ATTR_RW(brightness);

static int attiny_gpio_get_direction(struct gpio_chip *gc, unsigned int off)
{
    return GPIO_LINE_DIRECTION_OUT;
}

static int attiny_direction_input(struct gpio_chip *gc, unsigned int off)
{
    struct attiny_lcd *state = gpiochip_get_data(gc);
    int status;

    mutex_lock(&state->lock);
    status = attiny_set_bit(state, REG_IODIR, off, true);
    mutex_unlock(&state->lock);

    return status;
}

static int attiny_direction_output(struct gpio_chip *gc,
                                   unsigned int off,
                                   int value)
{
    struct attiny_lcd *state = gpiochip_get_data(gc);
    int status;
    u8 last_val;

    mutex_lock(&state->lock);
    status = attiny_set_bit(state, REG_IODIR, off, false);

    last_val = attiny_get_port_state(state, mappings[off].reg);
    if (value)
        last_val |= mappings[off].mask;
    else
        last_val &= ~mappings[off].mask;

    attiny_set_port_state(state, mappings[off].reg, last_val);
    mutex_unlock(&state->lock);

    return status;
}

static void attiny_gpio_set(struct gpio_chip *gc,
                            unsigned int off,
                            int val)
{
    struct attiny_lcd *state = gpiochip_get_data(gc);
    u8 last_val;

    if (off >= NUM_GPIO)
        return;

    mutex_lock(&state->lock);
    last_val = attiny_get_port_state(state, mappings[off].reg);
    if (val)
        last_val |= mappings[off].mask;
    else
        last_val &= ~mappings[off].mask;

    attiny_set_port_state(state, mappings[off].reg, last_val);
    mutex_unlock(&state->lock);
}

static int attiny_gpio_get(struct gpio_chip *gc, unsigned int off)
{
    struct attiny_lcd *state = gpiochip_get_data(gc);
    u8 last_val;
    int status;

    if (off >= NUM_GPIO)
        return -1;

    mutex_lock(&state->lock);
    last_val = attiny_get_port_state(state, mappings[off].reg);
    status = !!(last_val & BIT(off));
    mutex_unlock(&state->lock);

    return status;
}

/*
 * I2C driver interface functions
 */
static int attiny_i2c_probe(struct i2c_client *i2c)
{
    struct attiny_lcd *state;
    struct regmap *regmap;
    int ret;

    state = devm_kzalloc(&i2c->dev, sizeof(*state), GFP_KERNEL);
    if (!state)
        return -ENOMEM;

    mutex_init(&state->lock);
    i2c_set_clientdata(i2c, state);

    regmap = devm_regmap_init_i2c(i2c, &attiny_regmap_config);
    if (IS_ERR(regmap)) {
        ret = PTR_ERR(regmap);
        dev_err(&i2c->dev, "Failed to allocate register map: %d\n", ret);
        goto error;
    }
    state->regmap = regmap;

    /* Initialize default brightness to maximum */
    ret = regmap_write(state->regmap, REG_PWM, 0xff);
    if (ret) {
        dev_err(&i2c->dev, "Failed to set default brightness: %d\n", ret);
        goto error;
    }

    /* Create sysfs entry for brightness */
    ret = device_create_file(&i2c->dev, &dev_attr_brightness);
    if (ret) {
        dev_err(&i2c->dev, "Failed to create brightness sysfs: %d\n", ret);
        goto error;
    }

    /* Register GPIO chip */
    state->gc.parent = &i2c->dev;
    state->gc.label  = i2c->name;
    state->gc.owner  = THIS_MODULE;
    state->gc.base   = -1;
    state->gc.ngpio  = NUM_GPIO;

    state->gc.set             = attiny_gpio_set;
    state->gc.get             = attiny_gpio_get;
    state->gc.get_direction   = attiny_gpio_get_direction;
    state->gc.direction_input = attiny_direction_input;
    state->gc.direction_output= attiny_direction_output;
    state->gc.can_sleep       = true;

    ret = devm_gpiochip_add_data(&i2c->dev, &state->gc, state);
    if (ret) {
        dev_err(&i2c->dev, "Failed to create gpiochip: %d\n", ret);
        device_remove_file(&i2c->dev, &dev_attr_brightness);
        goto error;
    }

    return 0;

error:
    mutex_destroy(&state->lock);
    return ret;
}

static void attiny_i2c_remove(struct i2c_client *client)
{
    device_remove_file(&client->dev, &dev_attr_brightness);
    mutex_destroy(&((struct attiny_lcd *)i2c_get_clientdata(client))->lock);
    return;
}

static const struct of_device_id attiny_dt_ids[] = {
    { .compatible = "edatec,disp-regulator" },
    { }
};
MODULE_DEVICE_TABLE(of, attiny_dt_ids);

static struct i2c_driver attiny_regulator_driver = {
    .driver = {
        .name           = "edatec_disp_regulator",
        .of_match_table = of_match_ptr(attiny_dt_ids),
    },
    .probe  = attiny_i2c_probe,
    .remove = attiny_i2c_remove,
};
module_i2c_driver(attiny_regulator_driver);

MODULE_AUTHOR("Adapted by Riccardo De Martis");
MODULE_DESCRIPTION("Regulator driver for Edatec HMI2002: manual brightness via sysfs");
MODULE_LICENSE("GPL v2");
